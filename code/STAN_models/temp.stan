...
// Decision about data:
    ...
    data {
      ...
      corr_matrix[2] R; // used to get the covariance terms between sigmaPre and sigmaPost for each county (cov=R[1,2] * sigmaPre * sigmaPost)
      ...
      for (i in 1:counties) {
        for (j in 1:timepointsPre) {
          newCasesPre[i, j] ~ poisson(lambdaPre[i, j]);
        }
        
        for (k in 1:timepointsPost) {
          newCasesPost[i, k] ~ poisson(lambdaPost[i, k]);
        }
      }
